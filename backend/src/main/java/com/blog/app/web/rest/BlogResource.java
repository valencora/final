...
/**
 * REST controller for managing {@link com.blog.app.domain.Blog}.
 */
@RestController
...
public class BlogResource {
    ...
    public ResponseEntity<BlogDTO> createBlog(@Valid @RequestBody BlogDTO blogDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/blogs/" + blogDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, blogDTO.getId().toString()))
            .body(blogDTO);
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<BlogDTO>> getAllBlogs(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
    ...
    /**
     * {@code GET  /blogs/handle/:handle} : get the blog by handle.
     *
     * @param handle the handle of the blogDTO to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the blogDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/handle/{handle}")
    public ResponseEntity<BlogDTO> getBlogByHandle(@PathVariable("handle") String handle) {
        LOG.debug("REST request to get Blog by handle : {}", handle);
        Optional<BlogDTO> blogDTO = blogService.findByHandle(handle);
        return ResponseUtil.wrapOrNotFound(blogDTO);
    }
